...
const FILE = 'C:/Users/Ilham_PC/Downloads/Template_Import_Visit.xlsx';
...
const distMap = new Map(distributors.map(d=>[(d.name||'').toLowerCase().trim(), d.id]));
...
rows.forEach((r, i) => {
  ...
  if (!T(r.pic_nama)) errs.push('pic_nama kosong');
  if (!T(r.pic_hp)) errs.push('pic_hp kosong');
  ...
  ok.push({ ln, row: {
    ...
    distributor_id: T(r.distributor) ? (distMap.get(T(r.distributor).toLowerCase()) || null) : null,
